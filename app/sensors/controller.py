...
def create_sensor(sensor: schemas.SensorCreate, db: Session = Depends(get_db), mongodb_client: MongoDBClient = Depends(get_mongodb_client)):
    ...
    return repository.create_sensor(db=db, mongo=mongodb_client, sensor=sensor)
...
def record_data(sensor_id: int, data: dict,db: Session = Depends(get_db) ,redis_client: RedisClient = Depends(get_redis_client)):
    db_sensor = repository.get_sensor(db, sensor_id)
    if db_sensor is None:
        raise HTTPException(status_code=404, detail="Sensor not found")
...
def get_data(sensor_id: int, db: Session = Depends(get_db) ,redis_client: RedisClient = Depends(get_redis_client)):    
    db_sensor = repository.get_sensor(db,sensor_id)
    if db_sensor:
        sensor_data = json.loads(repository.get_data(redis=redis_client, sensor_id=sensor_id))
